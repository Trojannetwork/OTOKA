/**
 * Dither.js — React Bits Dither background component
 * Variant: JavaScript (plain React.createElement, no JSX)
 * Dependencies: react, react-dom, three, @react-three/fiber, postprocessing, @react-three/postprocessing
 * Loaded via ES import maps from CDN — no build step required.
 */
...
float cnoise(vec2 P) {
  ...
  Pi = mod289(Pi); vec4 ix = Pi.xzxz; vec4 iy = Pi.yyww;
...
function RetroEffect({ colorNum, pixelSize }) {
  return React.createElement(WrappedRetro, { colorNum, pixelSize });
}
...
function DitheredWaves(props) {
  ...
  useFrame(({ clock }) => {
    ...
    if (!prevColorRef.current.every((v, i) => v === waveColor[i])) {
      u.waveColor.value.set(...waveColor);
      prevColorRef.current = [...waveColor];
    }
